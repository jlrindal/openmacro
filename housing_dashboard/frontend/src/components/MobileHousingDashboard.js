...
const MobileHousingDashboard = () => {
  ...
  return (
    <div className={`min-h-screen bg-white text-gray-800 p-4 ${isMobile ? 'text-2xl' : 'text-base'}`}>
      <div className="relative mb-6">
        <div className={`flex items-center bg-gray-100 rounded-lg border border-gray-200 ${isMobile ? 'h-24' : 'h-16'} relative`}>
          <Search size={isMobile ? 36 : 24} className="ml-4 text-gray-600" />
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div
          className={`bg-gray-100 rounded-lg ${isMobile ? 'p-8' : 'p-6'} flex items-center relative overflow-hidden`}
          style={{
            border: `4px solid ${outlineColor}`,
            transition: 'border-color 0.5s ease',
          }}
        >
          <MapPin size={isMobile ? 72 : 48} className="mr-6 text-gray-600" />
          <div>
        ...
        <div className={`bg-gray-100 rounded-lg ${isMobile ? 'p-8' : 'p-6'} flex items-center`}>
          <DollarSign size={isMobile ? 72 : 48} className="mr-6 text-gray-600" />
          <div>
        ...
        <div className={`bg-gray-100 rounded-lg ${isMobile ? 'p-8' : 'p-6'} flex items-center`}>
          <Home size={isMobile ? 72 : 48} className="mr-6 text-gray-600" />
          <div>
      ...
      <div className="space-y-6">
        <div className="text-center">
          <span className={`font-bold text-gray-800 ${isMobile ? 'text-7xl' : 'text-4xl'}`}>
            {score}
          </span>
          <span className={`font-bold text-gray-500 ${isMobile ? 'text-5xl' : 'text-2xl'}`}>
            {' '}/ 10
          </span>
        ...
        <div className="flex justify-center">
          <div className={`w-full max-w-md bg-gray-200 rounded-full ${isMobile ? 'h-12' : 'h-8'} overflow-hidden`}>
            <div
      ...
      <div className={`mt-6 w-full ${isMobile ? 'h-[600px]' : 'h-[300px]'}`}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart
            data={filteredData}
            margin={{ left: isMobile ? 120 : 100, right: 20, top: 20, bottom: isMobile ? 100 : 70 }}
          >
            <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
            <XAxis
              dataKey="date"
              stroke="#000"
              tick={{ fill: '#000', fontSize: isMobile ? 32 : 20 }}
              tickFormatter={(value) => {
            ...
            />
            <Tooltip
              contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: isMobile ? 24 : 12 }}
            ...
            <Line
              ...
              dot={{ fill: getLineColor(), r: isMobile ? 6 : 4 }}
            />
          </LineChart>
        </ResponsiveContainer>
      </div>
      <div className="text-right space-y-1">
        <p className={`${isMobile ? 'text-xl' : 'text-sm'} text-gray-500 italic`}>
          * Excludes property taxes, insurance, and other housing costs
        </p>
        <p className={`${isMobile ? 'text-xl' : 'text-sm'} text-gray-500`}>
