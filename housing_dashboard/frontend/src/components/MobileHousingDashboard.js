...
const MobileHousingDashboard = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetch('https://quantnomics-deployment-d88ae194be04.herokuapp.com/api/housing_data');
    ...
    <div className={`min-h-screen bg-white text-gray-800 p-4 ${isMobile ? 'text-2xl' : 'text-base'}`}>
      <div className="relative mb-6">
        <div className="flex items-center bg-gray-100 rounded-lg border border-gray-200 h-16 relative">
          ...
          <input
            ...
            className={`w-full p-4 bg-transparent focus:outline-none ${isMobile ? 'text-2xl' : 'text-base'}`}
          />
          {searchTerm && (
            <X
              size={isMobile ? 48 : 24}
          ...
          {showDropdown && (
            <div className="absolute top-full left-0 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 z-10 max-h-48 overflow-y-auto">
              {filteredLocations.map((location, index) => (
                ...
                >
                  <p className={`${isMobile ? 'text-2xl' : 'text-base'}`}>{location}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        >
          ...
          <div>
            <h3 className={`font-semibold ${isMobile ? 'text-3xl' : 'text-xl'}`}>Current Location</h3>
            <p className={`${isMobile ? 'text-4xl' : 'text-2xl'}`}>{selectedLocation}</p>
          </div>
        </div>
        ...
        <div className="bg-gray-100 rounded-lg p-6 flex items-center">
          ...
          <div>
            <h3 className={`font-semibold ${isMobile ? 'text-3xl' : 'text-xl'}`}>{currentYear} Median HHI</h3>
            <p className={`${isMobile ? 'text-4xl' : 'text-2xl'}`}>
        ...
        <div className="bg-gray-100 rounded-lg p-6 flex items-center">
          ...
          <div>
            <h3 className={`font-semibold ${isMobile ? 'text-3xl' : 'text-xl'}`}>{currentYear} Median Price</h3>
            <p className={`${isMobile ? 'text-4xl' : 'text-2xl'}`}>
      ...
      <div className="space-y-6">
        <div className="text-center">
          <span className={`font-bold text-gray-800 ${isMobile ? 'text-6xl' : 'text-4xl'}`}>
            {score}
          </span>
          <span className={`font-bold text-gray-500 ${isMobile ? 'text-4xl' : 'text-2xl'}`}>
            {' '}/ 10
          </span>
          <p className={`text-gray-600 mt-4 ${isMobile ? 'text-3xl' : 'text-xl'}`}>
      ...
      {filteredData.length > 0 && (
        <p className={`text-center mt-8 mb-4 ${isMobile ? 'text-2xl' : 'text-xl'} text-gray-700`}>
      ...
      <div className={`mt-6 w-full ${isMobile ? 'h-[600px]' : 'h-[400px]'}`}>
      ...
      <div className="text-right space-y-1">
        <p className={`${isMobile ? 'text-lg' : 'text-sm'} text-gray-500 italic`}>
          * Excludes property taxes, insurance, and other housing costs
        </p>
        <p className={`${isMobile ? 'text-lg' : 'text-sm'} text-gray-500`}>
